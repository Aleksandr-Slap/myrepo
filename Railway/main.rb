...
class Interface
  ...
  def delete_an_intermediate_station
    ...
    @route.intermediate_stations.each_with_index do |_station, index|
  ...
  def create_pass_train
    ...
    @trains << train
    @trains.each { |_train| puts "Создан пассажирский поезд под номером #{number}" }
  ...
  def create_cargo_train
    ...
    @trains << train
    @trains.each { |_train| puts "Создан почтовый поезд под номером #{number}" }
  rescue RuntimeError => e
    puts "RuntimeError: #{e.message}"
    retry if @number !~ FORMAT_NUMBER
  end
  ...
  def create_pass_vagon
    ...
    trains_with_index
    puts "Прицеплен вагон #{vagon.number}" if @train.add_vagon(vagon)
    puts 'Тип вагона и поезда не совпадают или скорость поезда не 0 км/ч' unless @train.add_vagon(vagon)
  end
  ...
  def show_train_vagon
    ...
    trains_with_index
    if @train.train_vagons.size.zero?
      puts 'У данного поезда нет прицепленых вагонов'
    else
      @train.all_vagons do |vagon|
        puts "Номер вагона: #{vagon.number}"
        puts "Свободный объём/места: #{vagon.places} Занятый объём/места: #{vagon.occupied_places}"
      end
    end
  end

  # rubocop:disable Metrics/AbcSize
  def back_station_show
